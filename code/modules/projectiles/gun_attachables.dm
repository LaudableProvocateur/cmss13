...
/obj/item/attachable/vulture_scope/proc/get_time_to_fire()
	if(!istype(loc, /obj/item/weapon/gun/bolt_action/vulture))
		return 0

	var/obj/item/weapon/gun/bolt_action/vulture/rifle = loc
	if(!rifle.last_fired)
		return 0
...
/obj/item/attachable/attached_gun/shotgun //basically, a masterkey
	...
	max_rounds = 5
	current_rounds = 5
	ammo = /datum/ammo/bullet/shell/gauge_12/buckshot/masterkey
	slot = "under"
...
/obj/item/attachable/attached_gun/shotgun/reload_attachment(obj/item/ammo_magazine/handful/mag, mob/user)
	if(istype(mag) && mag.flags_magazine & AMMUNITION_HANDFUL)
		if(mag.default_ammo == /datum/ammo/bullet/shell/gauge_12/buckshot)
			if(current_rounds >= max_rounds)
				to_chat(user, SPAN_WARNING("[src] is full."))
			else
				current_rounds++
